...
pub async fn run(_socket_path: String, config: Config) -> Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        let addr = config.daemon.tcp_address.clone().unwrap_or_else(|| "127.0.0.1:9876".to_string());
...
async fn run_unix(socket_path: String, config: Config) -> Result<()> {
    ...
    info!("Daemon listening on {} (max connections: {})", socket_path, max_connections);
...
async fn run_tcp(addr: String, config: Config) -> Result<()> {
    ...
    let max_connections = config.daemon.max_connections;
    info!("Daemon listening on {} (max connections: {})", addr, max_connections);
            ...
            Ok(Some(Ok(line))) => {
                ...
                let request: Request = match serde_json::from_str(&line) {
                    ...
                    Err(e) => {
                        ...
                        let error_response = crate::protocol::error_response(None, -32700, "Parse error");
                        // Get the inner stream for writing error response
                        if let Err(write_err) = write_with_timeout(framed.get_mut(), &error_response, timeout_duration).await {
                ...
                if let Err(e) = write_with_timeout(framed.get_mut(), &response, timeout_duration).await {
...
async fn write_with_timeout<S>(stream: &mut S, data: &[u8], timeout_duration: Duration) -> Result<()>
where
